...
SUBROUTINE INITIAL
 ...
 REAL (KIND = 8), ALLOCATABLE :: NODE_XYZ(:,:),XYZ_LOCAL(:,:)
...
! Compute the center of each face
 ...
 DO K = 1,CELL_TOT
 	...
 	ALLOCATE (CELL_COE(K)%MCF(NK,NK))
 	ALLOCATE (CELL_COE(K)%FA_COE(NK))
 END DO

 ! For boundary conditions
 ALLOCATE (QWELL(WELL_NUM),WELL_DEPTH(WELL_NUM))
...
! OPEN (993,FILE = 'CENTER_CAL.DAT')
 ...
 DO K = 1,CELL_TOT
 	...
 	FACE_NUM = NK
 	CALL MFD_CAL(CELL_COE(K)%MCF,X0,Y0,FACE_NUM,LINE_COR,KTH,CELL_COE(K)%FA_COE,K_SCHEME)
	DEALLOCATE (LINE_COR)
 END DO
! CLOSE (993)
