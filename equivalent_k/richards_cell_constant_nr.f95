 ...
 DO WHILE (ABS(ERRO1).GE.ERRO.AND.IL.LE.INTER_CONTROL)
	...
	DO K = 1,CELL_TOT
		IF (CELL_COE(K)%POINT_TYPE.NE.(-1)) THEN 
			...
			DO IK = 1,FACE_NUM   
				DO JK = 1,FACE_NUM
					M_LOCAL(JK) = M_LOCAL(JK)+CELL_COE(K)%MCF(IK,JK)*CELL_COE(K)%KTH 
				END DO
			END DO
			DO IK = 1,FACE_NUM
				...
				DO IK = 1,FACE_NUM
					...
					DO JK = 1,FACE_NUM
						...
						DO II = 1,FACE_NUM
							...
							CELL_COE(K)%MCF(IK,JK)*CELL_COE(K)%KTH*M_LOCAL(II)/M_SUM
						END DO
						FACE_COE(NK)%BZ = FACE_COE(NK)%BZ-BZ0*CELL_COE(K)%MCF(IK,JK)*CELL_COE(K)%KTH/M_SUM  	
					END DO
					DEALLOCATE (F_LINK)
				END DO
			END IF
...
! Compute Velocity on each edge
 DO K = 1,CELL_TOT
 	IF (CELL_COE(K)%POINT_TYPE.NE.(-1)) THEN 
 		FACE_NUM = CELL_COE(K)%POINT_SIZE
 		ALLOCATE (VX(FACE_NUM),VY(FACE_NUM))
 		VX = 0.0; VY = 0.0
 		DO IK = 1,FACE_NUM
 			M_SUM = 0.0
 			DO JK = 1,FACE_NUM
 				KK = CELL_COE(K)%POINT_INDEX(JK)
 				M_SUM = M_SUM+CELL_COE(K)%MCF(IK,JK)*CELL_COE(K)%KTH*(FACE_COE(KK)%P-CELL_COE(K)%HTH)
 			END DO
 			KK = CELL_COE(K)%POINT_INDEX(IK)
