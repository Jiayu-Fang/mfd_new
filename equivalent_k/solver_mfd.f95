SUBROUTINE MFD_CAL(MCF,X0,Y0,LINE_NUM,LINE_COR,KTH,FA_COE,K_SCHEME)
 USE POINT
 IMPLICIT NONE 
 INTEGER :: LINE_NUM
 ...
 DO IK = 1,LINE_NUM
 	...
 	IF (LLX.LT.0.0) THEN 
 		NX1 = -NX1; NY1 = -NY1
 	END IF
 !	IF (K_SCHEME.EQ.1.OR.K_SCHEME.EQ.2) THEN 
 !		FA_COE(IK) = ABS(LLX*LL)
 !	END IF
 	
 	XK = KTH(1,1)*NX1+KTH(1,2)*NY1
 	RC(IK,1) = XK*LL
...
! 	DO IK = 1,LINE_NUM
! 		DO KK = 1,2
! 			FA_COE(IK) = FA_COE(IK)+NC(IK,KK)*RC(IK,KK)
! 		END DO
! 	END DO
! END IF
...
! IF (K_SCHEME.EQ.3) THEN 
 	...
 	DO IK = 1,LINE_NUM
 		DO KK = 1,LINE_NUM
 			FA_COE(IK) = FA_COE(IK)+MCF(IK,KK)*MCF(IK,KK)
 		END DO
 	END DO
! END IF
 LAMAC = 0.0
 DO IK = 1,LINE_NUM
 	LAMAC = LAMAC+MCF(IK,IK)
 END DO
 LAMAC = LAMAC/2.0
 
 DO IK = 1,LINE_NUM
 	DO JK = 1,LINE_NUM
 		MCF(IK,JK) = MCF(IK,JK)+LAMAC*PC(IK,JK)*1.0
 	END DO
 END DO
