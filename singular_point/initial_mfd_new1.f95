...
! For finding the nearest points for pumping wells 
 ...
 REAL (KIND = 8), ALLOCATABLE :: NODE_COR(:,:)
 ...
 READ (1,*) PICARD_REL,C_COE  ! C_COE is the shape parameter for RBF
 READ (1,*) K_SCHEME
 READ (1,*) NAME_FILE
...
! Reverse this order 
 ...
 DO K = 1,FACE_TOT
 	...
 	ALLOCATE (FACE_COE(K)%NODE_INDEX(2))
 	LOCAL = 0
 END DO
 CLOSE (1)
 ...
 DO K = 1,CELL_TOT
 	...
 	ALLOCATE (CELL_COE(K)%NX(NK),CELL_COE(K)%NY(NK),CELL_COE(K)%AREA(NK))
...
! Compute the center of each face
 DO K = 1,FACE_TOT
 	...
 	FACE_COE(K)%NODE_INDEX(1) = IK; FACE_COE(K)%NODE_INDEX(2) = JK
 ...
 DO K = 1,CELL_TOT
 	...
 	ALLOCATE (FACE_INDEX(NK))
 	ALLOCATE (NODE_COR(NK,2))
 	FACE_INDEX = 0
 	FACE_INDEX(1) = 1
 	KK = CELL_COE(K)%POINT_INDEX(1)
 	NODE_COR(1,1) = FACE_COE(KK)%NODE_XYZ(1,1)
 	NODE_COR(1,2) = FACE_COE(KK)%NODE_XYZ(1,2)
 	
 	NODE_COR(2,1) = FACE_COE(KK)%NODE_XYZ(2,1)
 	NODE_COR(2,2) = FACE_COE(KK)%NODE_XYZ(2,2)
 	
 	JJ = FACE_COE(KK)%NODE_INDEX(2)
 	II = 1; K1 = 1; I1 = 1; I2 = 2
 	DO WHILE (II.LT.NK)
 		DO I = 2,NK
 			KK = CELL_COE(K)%POINT_INDEX(I)
 			IF (JJ.EQ.FACE_COE(KK)%NODE_INDEX(1).OR.&
 			    JJ.EQ.FACE_COE(KK)%NODE_INDEX(2)) THEN 
 				K1 = 0
 				DO K2 = 1,II
	 				IF (I.EQ.FACE_INDEX(K2)) THEN 
 						K1 = 1
 					END IF
 				END DO
 				IF (K1.EQ.0) THEN 
 		...
 		II = II+1
 		FACE_INDEX(II) = J
 		KK = CELL_COE(K)%POINT_INDEX(J)
 		IF (JJ.EQ.FACE_COE(KK)%NODE_INDEX(1)) THEN 
 			I1 = 2; I2 = 1
 		ELSE 
 			I1 = 1; I2 = 2
 		END IF
 		NODE_COR(II,1) = FACE_COE(KK)%NODE_XYZ(I2,1)
 		NODE_COR(II,2) = FACE_COE(KK)%NODE_XYZ(I2,2)
 		
 		JJ = FACE_COE(KK)%NODE_INDEX(I1)
 	END DO
 
 	RNC_V = 0.0; RNC_VX = 0.0; RNC_VY = 0.0
 	
 	DO IK = 1,NK
 		X1 = NODE_COR(IK,1); Y1 = NODE_COR(IK,2)
 		IF (IK.LT.NK) THEN 
 			X2 = NODE_COR(IK+1,1); Y2 = NODE_COR(IK+1,2)
 		ELSE 
 			X2 = NODE_COR(1,1); Y2 = NODE_COR(1,2)
 		END IF
 		...
 		II = FACE_INDEX(IK)
 		KK = CELL_COE(K)%POINT_INDEX(II)
 		FACE_COE(KK)%AREA = LL
 		CELL_COE(K)%AREA(II) = LL
 		
 		NX1 = -NY; NY1 = NX
 		CELL_COE(K)%NX(II) = NX1
 		CELL_COE(K)%NY(II) = NY1
 	...
 	CELL_COE(K)%CELL_V = ABS(RNC_V)
 	IF (RNC_V.LT.0.0) THEN 
 		DO IK = 1,NK
 			CELL_COE(K)%NX(IK) = -CELL_COE(K)%NX(IK)
 			CELL_COE(K)%NY(IK) = -CELL_COE(K)%NY(IK)
 			II = FACE_INDEX(IK)
 			KK = CELL_COE(K)%POINT_INDEX(II)
 			FACE_COE(KK)%NX = -FACE_COE(KK)%NX
 			FACE_COE(KK)%NY = -FACE_COE(KK)%NY
 		END DO
 	END IF
...
! ALLOCATE (XYZ_WELL(WELL_NUM,3))
 ...
 ELSE 
 	...
 	OPEN (1,FILE = trim(NAME1))
 	READ (1,*) DRAW_NUM
 	ALLOCATE (DRAW_XYZ(DRAW_NUM),DRAW_LOCAL(DRAW_NUM,2))
 	DO I = 1,DRAW_NUM
 		READ (1,*) DRAW_LOCAL(I,1),DRAW_LOCAL(I,2)
 	END DO
 	CLOSE (1)
 	DO I = 1,DRAW_NUM
 		XK = 1000000.0
 		X0 = DRAW_LOCAL(I,1); Y0 = DRAW_LOCAL(I,2)
 		DRAW_XYZ(I) = 0
 		DO K = 1,CELL_TOT
 			X1 = CELL_COE(K)%X; Y1 = CELL_COE(K)%Y
 			LL = (X1-X0)**2+(Y1-Y0)**2
 			LL = SQRT(LL)
 			IF (LL.LT.XK) THEN 
 				XK = LL; DRAW_XYZ(I) = K
...
! OPEN (993,FILE = 'CENTER_CAL.DAT')
 ...
 DO K = 1,CELL_TOT
 	...
 	CALL MFD_CAL(CELL_COE(K)%MCF1,CELL_COE(K)%PC,X0,Y0,FACE_NUM,LINE_COR,KTH,CELL_COE(K)%FA_COE, &
 		     CELL_COE(K)%RRNC,CELL_COE(K)%NX,CELL_COE(K)%NY)
	DEALLOCATE (LINE_COR)
 END DO
! CLOSE (993)
...
! CLOSE (1)
 ...
 !	JK = FACE_NUM+3
 !	CALL RBF_GAU(X0,Y0,FACE_NUM,JK,XYZ_LOCAL,SHAPE_COE,C_COE)
 	JK = 3
 	CALL RBF_GAU1(X0,Y0,FACE_NUM,JK,XYZ_LOCAL,SHAPE_COE,C_COE)
 !	CALL GMLS(X0,Y0,FACE_NUM,JK,XYZ_LOCAL,SHAPE_COE)
 !	CALL IMLS(X0,Y0,FACE_NUM,XYZ_LOCAL,SHAPE_COE)
