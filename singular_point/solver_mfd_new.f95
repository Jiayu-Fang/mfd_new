SUBROUTINE MFD_CAL(MCF,PC,X0,Y0,LINE_NUM,LINE_COR,KTH,FA_COE,RRNC,NX_N,NY_N)
 USE POINT
 IMPLICIT NONE 
 INTEGER :: LINE_NUM
 TYPE (FACE_CAL_COR) :: LINE_COR(LINE_NUM)
 INTEGER :: IK,JK,KK
 INTEGER :: K1,K2,I,J,K
 REAL (KIND = 8) :: RH_INT,XK
 REAL (KIND = 8) :: KTH(2,2)
 REAL (KIND = 8) :: NC(LINE_NUM,2),RC(LINE_NUM,2),MCF(LINE_NUM,LINE_NUM),PC(LINE_NUM,LINE_NUM),RNC(2,2)
 REAL (KIND = 8) :: NX_N(LINE_NUM),NY_N(LINE_NUM)
 REAL (KIND = 8) :: RRNC(LINE_NUM,2)
 REAL (KIND = 8) :: NX,NY,X0,Y0,X1,Y1,X2,Y2
 REAL (KIND = 8) :: NX0,NY0,NX1,NY1,LLX,LLY,LL,DIS
 REAL (KIND = 8) :: AL1,AL2,LAMAC
 REAL (KIND = 8) :: FA_COE(LINE_NUM)
 
 DO IK = 1,LINE_NUM
 	LINE_COR(IK)%NODE_XYZ(1,1) = LINE_COR(IK)%NODE_XYZ(1,1)-X0
 	LINE_COR(IK)%NODE_XYZ(1,2) = LINE_COR(IK)%NODE_XYZ(1,2)-Y0
 	
 	LINE_COR(IK)%NODE_XYZ(2,1) = LINE_COR(IK)%NODE_XYZ(2,1)-X0
 	LINE_COR(IK)%NODE_XYZ(2,2) = LINE_COR(IK)%NODE_XYZ(2,2)-Y0
 END DO
 
 X0 = 0.0; Y0 = 0.0
 NC = 0.0; RC = 0.0
 DO IK = 1,LINE_NUM
 	X1 = LINE_COR(IK)%NODE_XYZ(1,1); Y1 = LINE_COR(IK)%NODE_XYZ(1,2)
 	X2 = LINE_COR(IK)%NODE_XYZ(2,1); Y2 = LINE_COR(IK)%NODE_XYZ(2,2)
 	
 	NX = X2-X1; NY = Y2-Y1
 	LL = SQRT(NX*NX+NY*NY)
 	
 	NX1 = NX_N(IK); NY1 = NY_N(IK)
 	 	
 	XK = KTH(1,1)*NX1+KTH(2,1)*NY1
 	RC(IK,1) = XK*LL
 	XK = KTH(2,1)*NX1+KTH(2,2)*NY1
 	RC(IK,2) = XK*LL
 	
 	K1 = 1; K2 = 0
 	CALL LINE_CAL(X1,Y1,X2,Y2,K1,K2,LLX)
 	NC(IK,1) = LLX/LL
 	K1 = 0; K2 = 1
 	CALL LINE_CAL(X1,Y1,X2,Y2,K1,K2,LLX)
 	NC(IK,2) = LLX/LL
 END DO
 
 RNC = 0.0
 DO IK = 1,2
 	DO KK = 1,2
 		DO JK = 1,LINE_NUM
 			RNC(IK,KK) = RNC(IK,KK)+RC(JK,IK)*NC(JK,KK)
 		END DO
 	END DO
 END DO
 
 IK = LINE_NUM; JK = 2
 CALL MFD_SOLVER(IK,NC,RC,RNC,JK,MCF,PC,RRNC)
 

 
END SUBROUTINE

SUBROUTINE LINE_CAL(X1,Y1,X2,Y2,K1,K2,LLX)
 IMPLICIT NONE
 REAL (KIND = 8) :: X1,Y1,X2,Y2,NX,NY,LLX,TM
 REAL (KIND = 8) :: KK,NXY,XK,YK,LOW_LINE,UP_LINE
 REAL (KIND = 8) :: KX,KY
 INTEGER :: K1,K2
 
 NX = X2-X1
 NY = Y2-Y1
 
 IF (ABS(NX).GT.1.0E-6) THEN 
 	NXY = NY/NX
 	IF (X2.LT.X1) THEN 
 		XK = X2; YK = Y2
 		X2 = X1; Y2 = Y1
 		X1 = XK; Y1 = YK
 	END IF
 	TM = 0.0
 	KX = 1.0; KY = NXY
 	CALL POLY_COE(K1,K2,X1,Y1,KX,KY,TM,LOW_LINE)
 	TM = X2-X1
 	CALL POLY_COE(K1,K2,X1,Y1,KX,KY,TM,UP_LINE)
 	LLX = (UP_LINE-LOW_LINE)*SQRT(NXY*NXY+1.0)
 ELSE 
 	NXY = NX/NY
 	IF (Y2.LT.Y1) THEN 
  		XK = X2; YK = Y2
 		X2 = X1; Y2 = Y1
 		X1 = XK; Y1 = YK
 	END IF
 	TM = 0.0
 	KX = NXY; KY = 1.0
 	CALL POLY_COE(K1,K2,X1,Y1,KX,KY,TM,LOW_LINE)
 	TM = Y2-Y1
 	CALL POLY_COE(K1,K2,X1,Y1,KX,KY,TM,UP_LINE)
 	LLX = (UP_LINE-LOW_LINE)*SQRT(NXY*NXY+1.0)
 END IF

END SUBROUTINE

SUBROUTINE MFD_SOLVER(FACE_NUM,NC,RC,RNC,ORD_NUM,MCF,PC,RRNC)
 IMPLICIT NONE 
 INTEGER :: FACE_NUM,ORD_NUM
 INTEGER :: IK,JK,KK
 REAL (KIND = 8) :: NC(FACE_NUM,ORD_NUM),RC(FACE_NUM,ORD_NUM)
 REAL (KIND = 8) :: RNC(ORD_NUM,ORD_NUM),RRNC(FACE_NUM,ORD_NUM),MCF(FACE_NUM,FACE_NUM)
 REAL (KIND = 8) :: IRNC(ORD_NUM,ORD_NUM),INNC(ORD_NUM,ORD_NUM),PC(FACE_NUM,FACE_NUM)
 REAL (KIND = 8) :: LAMAC,NNC(ORD_NUM,ORD_NUM),NNNC(FACE_NUM,ORD_NUM),MCFP(FACE_NUM,FACE_NUM)
 REAL (KIND = 8) :: PC1(FACE_NUM,FACE_NUM)

 ! Inverse of RNC
 CALL LU_INVERSE(ORD_NUM,RNC,IRNC)
 
 ! Compute DG^(-1)
 RRNC = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,ORD_NUM
 		DO KK = 1,ORD_NUM
 			RRNC(IK,JK) = RRNC(IK,JK)+RC(IK,KK)*IRNC(KK,JK)
 		END DO
 	END DO 
 END DO
 ! Compute MCF
 MCF = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,FACE_NUM
 		DO KK = 1,ORD_NUM
 			MCF(IK,JK) = MCF(IK,JK)+RRNC(IK,KK)*RC(JK,KK)
 		END DO
 	END DO
 END DO 


! Compute NNNC
 NNNC = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,ORD_NUM
 		DO KK = 1,ORD_NUM
 			NNNC(IK,JK) = NNNC(IK,JK)+NC(IK,KK)*IRNC(KK,JK)
 		END DO
 	END DO
 END DO
! Compute MCFP
 MCFP = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,FACE_NUM
 		DO KK = 1,ORD_NUM
 			MCFP(IK,JK) = MCFP(IK,JK)+NNNC(IK,KK)*RC(JK,KK)
 		END DO
 	END DO
 END DO
 
 PC1 = -MCFP
 DO IK = 1,FACE_NUM
 	PC1(IK,IK) = PC1(IK,IK)+1.0
 END DO
 
 PC = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,FACE_NUM
 		DO KK = 1,FACE_NUM
 			PC(IK,JK) = PC(IK,JK)+PC1(KK,IK)*PC1(KK,JK)
 		END DO
 	END DO
 END DO
 
 
 
END SUBROUTINE 


SUBROUTINE POLY_COE(ORD1,ORD2,XF,YF,KX,KY,TM,R_INT)
 IMPLICIT NONE 
 INTEGER :: ORD1,ORD2
 INTEGER :: IK,JK,KK,N,I,J,K
 INTEGER :: K1,K2,K3,N1,N2,N3,NN,IK1,IK2,JK1,JK2,KK1,KK2
 REAL (KIND = 8) :: XF,YF,KX,KY,R_INT,TM
 REAL (KIND = 8) :: POLY_X(0:ORD1),POLY_Y(0:ORD2)
 REAL (KIND = 8) :: POLY_XY(0:ORD1+ORD2)
 
 POLY_X = 0.0; POLY_Y = 0.0
 POLY_XY = 0.0
 
 DO IK = 0,ORD1
 	JK = ORD1-IK
 	N1 = 1; N2 = 1; NN = 1
 	DO I = 1,ORD1
 		NN = NN*I
 	END DO
 	DO I = 1,IK
 		N1 = N1*I
 	END DO
 	DO I = 1,JK
 		N2 = N2*I
 	END DO
 	POLY_X(IK) = POLY_X(IK)+NN/N1/N2*1.0*(XF**JK)*(KX**IK)
 END DO
 DO IK = 0,ORD2
 	JK = ORD2-IK
 	N1 = 1; N2 = 1; NN = 1
 	DO I = 1,ORD2
 		NN = NN*I
 	END DO
 	DO I = 1,IK
 		N1 = N1*I
 	END DO
 	DO I = 1,JK
 		N2 = N2*I
 	END DO
 	POLY_Y(IK) = POLY_Y(IK)+NN/N1/N2*1.0*(YF**JK)*(KY**IK)
 END DO
 DO IK = 0,ORD1
 	DO JK = 0,ORD2
 		POLY_XY(IK+JK) = POLY_XY(IK+JK)+POLY_X(IK)*POLY_Y(JK)
 	END DO
 END DO
 R_INT = 0.0
 DO IK = 0,ORD1+ORD2
 	R_INT = R_INT+POLY_XY(IK)*(TM**(IK+1))/(IK+1.0)
 END DO
 
END SUBROUTINE POLY_COE

SUBROUTINE MFD_SOLVER_NN(FACE_NUM,NC,RC,RNC,ORD_NUM,MCF,PC,RRNC)
 IMPLICIT NONE 
 INTEGER :: FACE_NUM,ORD_NUM
 INTEGER :: IK,JK,KK
 REAL (KIND = 8) :: NC(FACE_NUM,ORD_NUM),RC(FACE_NUM,ORD_NUM)
 REAL (KIND = 8) :: RNC(ORD_NUM,ORD_NUM),RRNC(FACE_NUM,ORD_NUM),MCF(FACE_NUM,FACE_NUM)
 REAL (KIND = 8) :: IRNC(ORD_NUM,ORD_NUM),INNC(ORD_NUM,ORD_NUM),PC(FACE_NUM,FACE_NUM)
 REAL (KIND = 8) :: LAMAC,NNC(ORD_NUM,ORD_NUM),NNNC(FACE_NUM,ORD_NUM),MCFP(FACE_NUM,FACE_NUM)
 REAL (KIND = 8) :: NC_T(FACE_NUM,ORD_NUM),NC_B(FACE_NUM,ORD_NUM)

 ! Inverse of RNC
 CALL LU_INVERSE(ORD_NUM,RNC,IRNC)
 
 ! Compute DG^(-1)
 RRNC = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,ORD_NUM
 		DO KK = 1,ORD_NUM
 			RRNC(IK,JK) = RRNC(IK,JK)+RC(IK,KK)*IRNC(KK,JK)
 		END DO
 	END DO 
 END DO
 ! Compute MCF
 MCF = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,FACE_NUM
 		DO KK = 1,ORD_NUM
 			MCF(IK,JK) = MCF(IK,JK)+RRNC(IK,KK)*RC(JK,KK)
 		END DO
 	END DO
 END DO 

 NNC = 0.0
 DO IK = 1,ORD_NUM
 	DO KK = 1,ORD_NUM
 		DO JK = 1,FACE_NUM	
 			NNC(IK,KK) = NNC(IK,KK)+NC(JK,IK)*NC(JK,KK)
 		END DO
 	END DO 
 END DO
 
 ! Inverse of NNC
 CALL LU_INVERSE(ORD_NUM,NNC,INNC)
 ! Compute NNNC
 NNNC = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,ORD_NUM
 		DO KK = 1,ORD_NUM
 			NNNC(IK,JK) = NNNC(IK,JK)+NC(IK,KK)*INNC(KK,JK)
 		END DO
 	END DO
 END DO
 ! Compute MCFP
 MCFP = 0.0
 DO IK = 1,FACE_NUM
 	DO JK = 1,FACE_NUM
 		DO KK = 1,ORD_NUM
 			MCFP(IK,JK) = MCFP(IK,JK)+NNNC(IK,KK)*NC(JK,KK)  
 		END DO
 	END DO
 END DO
 
 PC = -MCFP
 DO IK = 1,FACE_NUM
 	PC(IK,IK) = PC(IK,IK)+1.0
 END DO
 
END SUBROUTINE 

SUBROUTINE LU_INVERSE(RF_NUM,A,A_INV)
 IMPLICIT NONE 
 INTEGER :: RF_NUM
 REAL (KIND = 8) :: A(RF_NUM,RF_NUM),A_INV(RF_NUM,RF_NUM),A_TEM(RF_NUM)
 REAL (KIND = 8) :: L(RF_NUM,RF_NUM),U(RF_NUM,RF_NUM),LI(RF_NUM,RF_NUM),UI(RF_NUM,RF_NUM)
 REAL (KIND = 8) :: XK
 INTEGER :: IK,JK,KK,NK_B
 INTEGER :: IND_B(RF_NUM,2)
 
 NK_B = 0; IND_B = 0
 DO IK = 1,RF_NUM
 	IF (ABS(A(IK,IK)).LT.1.0E-15) THEN 
 		JK = IK+1
 		DO WHILE (ABS(A(JK,IK)).LT.1.0E-15.AND.JK.LT.RF_NUM)
 			JK = JK+1
 		END DO			
 		DO KK = 1,RF_NUM
 			A_TEM(KK) = A(JK,KK)
 		END DO
 		DO KK = 1,RF_NUM
 			A(JK,KK) = A(IK,KK)
 		END DO
 		DO KK = 1,RF_NUM
 			A(IK,KK) = A_TEM(KK)
 		END DO
 		NK_B = NK_B+1
 		IND_B(NK_B,1) = JK; IND_B(NK_B,2) = IK
 	END IF
 END DO
 
 
 L = 0.0; U = 0.0
 DO IK = 1,RF_NUM
 	JK = IK
 	L(IK,JK) = 1.0
 END DO
 
! LU Decomposition
 DO JK = 1,RF_NUM
 	U(1,JK) = A(1,JK)
 END DO
 DO IK = 2,RF_NUM
 	L(IK,1) = A(IK,1)/U(1,1)
 	DO JK = 2,IK-1
 		XK = A(IK,JK)
 		DO KK = 1,JK-1
 			XK = XK-L(IK,KK)*U(KK,JK)
 		END DO
 		IF (ABS(U(JK,JK)).LT.1.0E-18) THEN 
 			WRITE (*,*) 'WARNING'
 			L(IK,JK) = 0.0
 		ELSE 
 			L(IK,JK) = XK/U(JK,JK)
 		END IF
 	END DO
 	DO JK = IK,RF_NUM
 		XK = A(IK,JK)
 		DO KK = 1,JK-1
 			XK = XK-L(IK,KK)*U(KK,JK)
 		END DO
 		U(IK,JK) = XK/1.0
 	END DO
 END DO
 
! Computing the inverse of L AND U MATRIX
! For U
 LI = 0.0; UI = 0.0
 DO IK = 1,RF_NUM
 	DO JK = 1,RF_NUM
 		IF (IK.EQ.JK) THEN 
 			XK = 1.0
 		ELSE 
 			XK = 0.0
 		END IF
 		DO KK = 1,JK-1
 			XK = XK-UI(IK,KK)*U(KK,JK)
 		END DO
 		IF (ABS(U(JK,JK)).LT.1.0E-18) THEN 
 			UI(IK,JK) = 0.0
 		ELSE  
 			UI(IK,JK) = XK/U(JK,JK)
 		END IF
 	END DO
 END DO

! For L
 DO IK = 1,RF_NUM
 	DO JK = RF_NUM,1,-1
 		IF (IK.EQ.JK) THEN 
 			XK = 1.0
 		ELSE 
 			XK = 0.0
 		END IF
 		DO KK = RF_NUM,JK+1,-1
 			XK = XK-LI(IK,KK)*L(KK,JK)
 		END DO
 		LI(IK,JK) = XK
 	END DO
 END DO

! For the final inverse of A
 A_INV = 0.0
 DO IK = 1,RF_NUM
 	DO JK = 1,RF_NUM
 		DO KK = 1,RF_NUM
 			A_INV(IK,JK) = A_INV(IK,JK)+UI(IK,KK)*LI(KK,JK)
 		END DO
 	END DO
 END DO

 DO IK = RF_NUM,1,-1
 	IF (IND_B(IK,1).NE.0) THEN 
 		DO KK = 1,RF_NUM
 			A_TEM(KK) = A_INV(KK,IND_B(IK,1))
 		END DO
 		DO KK = 1,RF_NUM
 			A_INV(KK,IND_B(IK,1)) = A_INV(KK,IND_B(IK,2)) 
 		END DO
 		DO KK = 1,RF_NUM
 			A_INV(KK,IND_B(IK,2)) = A_TEM(KK)
 		END DO
 	END IF
 END DO
 
 DO IK = RF_NUM,1,-1
 	IF (IND_B(IK,1).NE.0) THEN 
 		DO KK = 1,RF_NUM
 			A_TEM(KK) = A(KK,IND_B(IK,1))
 		END DO
 		DO KK = 1,RF_NUM
 			A(KK,IND_B(IK,1)) = A(KK,IND_B(IK,2)) 
 		END DO
 		DO KK = 1,RF_NUM
 			A(KK,IND_B(IK,2)) = A_TEM(KK)
 		END DO
 	END IF
 END DO
 
END SUBROUTINE 

SUBROUTINE RBF_GAU1(X0,Y0,XYZ_NUM,RBF_NUM,XYZ_L,SHAPE_COE,C_COE)   ! Directly solving the saddle point problem
 IMPLICIT NONE 
 INTEGER :: XYZ_NUM,RBF_NUM
 INTEGER :: IK,JK,KK
 REAL (KIND = 8) :: XYZ_L(XYZ_NUM,2)
 REAL (KIND = 8) :: AK(XYZ_NUM,XYZ_NUM),BK(RBF_NUM,XYZ_NUM)
 REAL (KIND = 8) :: AK_INV(XYZ_NUM,XYZ_NUM),BA(RBF_NUM,XYZ_NUM)
 REAL (KIND = 8) :: BAB(RBF_NUM,RBF_NUM),BAB_INV(RBF_NUM,RBF_NUM)
 REAL (KIND = 8) :: BT(XYZ_NUM,XYZ_NUM)
 REAL (KIND = 8) :: A_INV(XYZ_NUM,XYZ_NUM),B_INV(RBF_NUM,XYZ_NUM)
 REAL (KIND = 8) :: DX,DY,XK,YK,ZK,XM,YM,ZM,R_DIS
 REAL (KIND = 8) :: R_SM,X0,Y0
 REAL (KIND = 8) :: A_COE(XYZ_NUM+RBF_NUM)
 REAL (KIND = 8) :: C_COE
 REAL (KIND = 8) :: SHAPE_COE(XYZ_NUM)
 
! C_COE = 1.0
 
 DO IK = 1,XYZ_NUM
 	XYZ_L(IK,1) = XYZ_L(IK,1)-X0
 	XYZ_L(IK,2) = XYZ_L(IK,2)-Y0
 END DO
 X0 = 0.0; Y0 = 0.0
 
 R_SM = 0.0
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		DX = XYZ_L(IK,1)-XYZ_L(JK,1)
 		DY = XYZ_L(IK,2)-XYZ_L(JK,2)
 		R_DIS = SQRT(DX*DX+DY*DY)
 		R_SM = MAX(R_SM,R_DIS)
 	END DO
 END DO
 
 R_SM = R_SM*1.0
! R_SM = 1.0
 AK = 0.0; BK = 0.0
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		DX = XYZ_L(IK,1)-XYZ_L(JK,1)
 		DY = XYZ_L(IK,2)-XYZ_L(JK,2)
 		R_DIS = SQRT(DX*DX+DY*DY)/R_SM
 		AK(IK,JK) = EXP(-C_COE*R_DIS*R_DIS)
 	END DO
 END DO
 DO JK = 1,XYZ_NUM
 	BK(1,JK) = 1.0
 	BK(2,JK) = XYZ_L(JK,1)
 	BK(3,JK) = XYZ_L(JK,2)
 END DO
 
 CALL LU_INVERSE(XYZ_NUM,AK,AK_INV)
 BA = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,XYZ_NUM
 		DO KK = 1,XYZ_NUM
 			BA(IK,JK) = BA(IK,JK)+BK(IK,KK)*AK_INV(KK,JK)
 		END DO
 	END DO
 END DO
 
 BAB = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,RBF_NUM
 		DO KK = 1,XYZ_NUM
 			BAB(IK,JK) = BAB(IK,JK)+BA(IK,KK)*BK(JK,KK)
 		END DO
 	END DO
 END DO
 
 CALL LU_INVERSE(RBF_NUM,BAB,BAB_INV)
 
 B_INV = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,XYZ_NUM
 		DO KK = 1,RBF_NUM
 			B_INV(IK,JK) = B_INV(IK,JK)+BAB_INV(IK,KK)*BA(KK,JK)
 		END DO
 	END DO
 END DO
 
 BT = 0.0
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		DO KK = 1,RBF_NUM
 			BT(IK,JK) = BT(IK,JK)+BK(KK,IK)*B_INV(KK,JK)
 		END DO
 	END DO
 END DO
 
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		IF (IK.EQ.JK) THEN 
 			BT(IK,JK) = 1.0-BT(IK,JK)
 		ELSE 
 			BT(IK,JK) = -BT(IK,JK)
 		END IF
 	END DO
 END DO
 
 A_INV = 0.0
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		DO KK = 1,XYZ_NUM
 			A_INV(IK,JK) = A_INV(IK,JK)+AK_INV(IK,KK)*BT(KK,JK)
 		END DO
 	END DO
 END DO
 
 A_COE = 0.0
 DO IK = 1,XYZ_NUM
 	DX = XYZ_L(IK,1)-X0
 	DY = XYZ_L(IK,2)-Y0
 	R_DIS = SQRT(DX*DX+DY*DY)/R_SM
 	A_COE(IK) = EXP(-C_COE*R_DIS*R_DIS)
 END DO
 A_COE(XYZ_NUM+1) = 1.0; A_COE(XYZ_NUM+2) = X0
 A_COE(XYZ_NUM+3) = Y0
 
 SHAPE_COE = 0.0
 DO IK = 1,XYZ_NUM
 	DO KK = 1,XYZ_NUM
 		SHAPE_COE(IK) = SHAPE_COE(IK)+A_INV(IK,KK)*A_COE(KK)
 	END DO
 	DO KK = 1,RBF_NUM
 		SHAPE_COE(IK) = SHAPE_COE(IK)+B_INV(KK,IK)*A_COE(KK+XYZ_NUM)
 	END DO
 END DO

END SUBROUTINE 


SUBROUTINE RBF_GAU(X0,Y0,XYZ_NUM,RBF_NUM,XYZ_L,SHAPE_COE,C_COE)
 IMPLICIT NONE 
 INTEGER :: XYZ_NUM,RBF_NUM
 INTEGER :: IK,JK,KK
 REAL (KIND = 8) :: XYZ_L(XYZ_NUM,2),AK(RBF_NUM,RBF_NUM)
 REAL (KIND = 8) :: A(RBF_NUM,RBF_NUM),A_INV_RBF(RBF_NUM,RBF_NUM)
 REAL (KIND = 8) :: DX,DY,XK,YK,ZK,XM,YM,ZM,R_DIS
 REAL (KIND = 8) :: R_SM,X0,Y0
 REAL (KIND = 8) :: M_RESULT(RBF_NUM,RBF_NUM),A_COE(RBF_NUM)
 REAL (KIND = 8) :: C_COE
 REAL (KIND = 8) :: SHAPE_COE(XYZ_NUM)
 
! C_COE = 1.0
 
 DO IK = 1,XYZ_NUM
 	XYZ_L(IK,1) = XYZ_L(IK,1)-X0
 	XYZ_L(IK,2) = XYZ_L(IK,2)-Y0
 END DO
 X0 = 0.0; Y0 = 0.0
 
 R_SM = 0.0
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		DX = XYZ_L(IK,1)-XYZ_L(JK,1)
 		DY = XYZ_L(IK,2)-XYZ_L(JK,2)
 		R_DIS = SQRT(DX*DX+DY*DY)
 		R_SM = MAX(R_SM,R_DIS)
 	END DO
 END DO
 
! R_SM = R_SM*1.1
 R_SM = 1.0
 AK = 0.0; A = 0.0
 DO IK = 1,XYZ_NUM
 	DO JK = 1,XYZ_NUM
 		DX = XYZ_L(IK,1)-XYZ_L(JK,1)
 		DY = XYZ_L(IK,2)-XYZ_L(JK,2)
 		R_DIS = SQRT(DX*DX+DY*DY)/R_SM
 		AK(IK,JK) = EXP(-C_COE*R_DIS*R_DIS)
 	END DO
 	AK(IK,XYZ_NUM+1) = 1.0
 	AK(IK,XYZ_NUM+2) = XYZ_L(IK,1)
 	AK(IK,XYZ_NUM+3) = XYZ_L(IK,2)
 END DO
 DO JK = 1,XYZ_NUM
 	AK(XYZ_NUM+1,JK) = 1.0
 	AK(XYZ_NUM+2,JK) = XYZ_L(JK,1)
 	AK(XYZ_NUM+3,JK) = XYZ_L(JK,2)
 END DO
 
 A = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,RBF_NUM
 		DO KK = 1,RBF_NUM
 			A(IK,JK) = A(IK,JK)+AK(IK,KK)*AK(KK,JK)
 		END DO
 	END DO
 END DO
 
 CALL LU_INVERSE (RBF_NUM,A,A_INV_RBF)
 
 M_RESULT = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,RBF_NUM
 		DO KK = 1,RBF_NUM
 			M_RESULT(IK,JK) = M_RESULT(IK,JK)+A_INV_RBF(IK,KK)*AK(KK,JK)
 		END DO
 	END DO	
 END DO
 DO IK = 1,RBF_NUM
 	DO JK = 1,RBF_NUM
 		A_INV_RBF(IK,JK) = M_RESULT(IK,JK)
 	END DO
 END DO
 A_COE = 0.0
 DO IK = 1,XYZ_NUM
 	DX = XYZ_L(IK,1)-X0
 	DY = XYZ_L(IK,2)-Y0
 	R_DIS = SQRT(DX*DX+DY*DY)/R_SM
 	A_COE(IK) = EXP(-C_COE*R_DIS*R_DIS)
 END DO
 A_COE(XYZ_NUM+1) = 1.0; A_COE(XYZ_NUM+2) = X0
 A_COE(XYZ_NUM+3) = Y0
 
 SHAPE_COE = 0.0
 DO IK = 1,XYZ_NUM
 	DO KK = 1,RBF_NUM
 		SHAPE_COE(IK) = SHAPE_COE(IK)+A_INV_RBF(IK,KK)*A_COE(KK)
 	END DO
 END DO

END SUBROUTINE 

SUBROUTINE IMLS(X0,Y0,XYZ_NUM,XYZ_L,SHAPE_COE)
 IMPLICIT NONE 
 INTEGER :: XYZ_NUM
 INTEGER :: IK,JK,KK
 REAL (KIND = 8) :: XYZ_L(XYZ_NUM,2)
 REAL (KIND = 8) :: DX,DY,XK,YK,ZK,XM,YM,ZM,R_DIS
 REAL (KIND = 8) :: R_SM,X0,Y0
 REAL (KIND = 8) :: SHAPE_COE(XYZ_NUM)
 
 SHAPE_COE = 0.0
 DO IK = 1,XYZ_NUM
 	XK = 1.0; YK = 1.0
 	DO JK = 1,XYZ_NUM
 		IF (JK.NE.IK) THEN 
 			DX = XYZ_L(JK,1)-X0
 			DY = XYZ_L(JK,2)-Y0
 			R_DIS = DX**2+DY**2
 			XK = XK*R_DIS
 			DX = XYZ_L(JK,1)-XYZ_L(IK,1)
 			DY = XYZ_L(JK,2)-XYZ_L(IK,2)
 			R_DIS = DX**2+DY**2
 			YK = YK*R_DIS
 		END IF
 	END DO
 	SHAPE_COE(IK) = XK/YK
 END DO
 XK = 0.0
 DO IK = 1,XYZ_NUM
 	XK = XK+SHAPE_COE(IK)
 END DO
 DO IK = 1,XYZ_NUM
 	SHAPE_COE(IK) = SHAPE_COE(IK)/XK
 END DO
 

END SUBROUTINE IMLS


SUBROUTINE GMLS(X0,Y0,XYZ_NUM,RBF_NUM,XYZ_L,SHAPE_COE)
 IMPLICIT NONE 
 INTEGER :: XYZ_NUM,RBF_NUM
 INTEGER :: IK,JK,KK
 REAL (KIND = 8) :: XYZ_L(XYZ_NUM,2),AK(XYZ_NUM,RBF_NUM)
 REAL (KIND = 8) :: A(RBF_NUM,RBF_NUM),A_INV_RBF(RBF_NUM,RBF_NUM)
 REAL (KIND = 8) :: DX,DY,XK,YK,ZK,XM,YM,ZM,R_DIS
 REAL (KIND = 8) :: R_SM,X0,Y0
 REAL (KIND = 8) :: M_RESULT(RBF_NUM,XYZ_NUM)
 REAL (KIND = 8) :: WL(XYZ_NUM)
 REAL (KIND = 8) :: WLD
 REAL (KIND = 8) :: SHAPE_COE(XYZ_NUM)
 
 R_SM = 0.0
 DO IK = 1,XYZ_NUM
 	DX = XYZ_L(IK,1)-X0
 	DY = XYZ_L(IK,2)-Y0
 	R_DIS = SQRT(DX*DX+DY*DY)
 	R_SM = MAX(R_SM,R_DIS)
 END DO
 
 R_SM = R_SM*1.1
 WL = 0.0
 DO IK = 1,XYZ_NUM
 	DX = XYZ_L(IK,1)-X0
 	DY = XYZ_L(IK,2)-Y0
 	R_DIS = SQRT(DX*DX+DY*DY)
 	CALL WL_CAL(R_DIS,R_SM,WL(IK))
 END DO
 
 AK = 0.0; A = 0.0
 DO IK = 1,XYZ_NUM
 	DX = (XYZ_L(IK,1)-X0)/R_SM
 	DY = (XYZ_L(IK,2)-Y0)/R_SM
 	AK(IK,1) = 1.0; AK(IK,2) = DX
 	AK(IK,3) = DY
 END DO
 
 A = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,RBF_NUM
 		DO KK = 1,XYZ_NUM
 			A(IK,JK) = A(IK,JK)+AK(KK,IK)*AK(KK,JK)*WL(KK)
 		END DO
 	END DO
 END DO
 
 CALL LU_INVERSE(RBF_NUM,A,A_INV_RBF)
 
 M_RESULT = 0.0
 SHAPE_COE = 0.0
 DO IK = 1,RBF_NUM
 	DO JK = 1,XYZ_NUM
 		DO KK = 1,RBF_NUM
 			M_RESULT(IK,JK) = M_RESULT(IK,JK)+A_INV_RBF(IK,KK)*WL(JK)*AK(JK,KK)
 		END DO
 	END DO
 END DO
 
 DO IK = 1,XYZ_NUM
 	SHAPE_COE(IK) = M_RESULT(1,IK)
 END DO

END SUBROUTINE 



SUBROUTINE WL_CAL(RR,RM,WLR)
 IMPLICIT NONE 
 REAL (KIND = 8) :: RM,WLR,RR
 REAL (KIND = 8) :: XK
 
 WLR = 0.0
 IF (RR.LT.RM) THEN 
 	XK = (1.0-RM/RR)**2
 	WLR = (RM**2)/(RR**2)*XK
 END IF

END SUBROUTINE 


























