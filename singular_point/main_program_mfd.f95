...
MODULE MATRIX
 ...
 ! For soil properties
 	...
 	REAL (KIND = 8), ALLOCATABLE :: MCF(:,:),MCF1(:,:),PC(:,:)
	...
	!	WRITE (993,'(100F20.10)') T,(HTH(DRAW_XYZ(N)),N=1,DRAW_NUM)
		...
		ELSE 
			...
			!	IF (T.LE.OUTPUT_VEC(IK).AND.(T+DT).GT.OUTPUT_VEC(IK)) THEN
				...
				!	WRITE (993,'(70000F20.10)') T,(CELL_COE((DRAW_XYZ(N)))%HTH,N=1,DRAW_NUM)
				!	WRITE (994,'(70000F20.10)') T,(CELL_COE((DRAW_XYZ(N)))%MTH,N=1,DRAW_NUM)
					OPEN (911,FILE = trim(PATH)//trim(NAME_FILE)//'_head_'//CHAR(IK+48)//'.dat')
					OPEN (995,FILE = trim(PATH)//trim(NAME_FILE)//'_velocity_'//CHAR(IK+48)//'.dat',status = 'unknown')
					DO N = 1,DRAW_NUM
						WRITE (911,'(I10,20F20.10)') N,CELL_COE(N)%X,CELL_COE(N)%Y,CELL_COE(N)%HTH,CELL_COE(N)%CELL_V
