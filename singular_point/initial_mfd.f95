...
! CLOSE (1)
 ...
 DO IK = 1,WELL_NUM
 	...
 	DO JK = 1,WELL_COE(IK)%CELL_NUM  ! COEFFICIENTS FOR THE CELL NODES FIRST
 		...
 		XYZ_LOCAL(JK,1) = NODE_CF(II)%X
 		XYZ_LOCAL(JK,2) = NODE_CF(II)%Y
 	END DO	
 	NK = WELL_COE(IK)%CELL_NUM
 	DO JK = 1,WELL_COE(IK)%FACE_NUM
 		II = WELL_COE(IK)%WELL_FACE_INDEX(JK)
 		XYZ_LOCAL(JK+NK,1) = NODE_CF(II)%X
 		XYZ_LOCAL(JK+NK,2) = NODE_CF(II)%Y
 	END DO
 ...
 DO IK = 1,WELL_NUM
 	...
 	DO JK = 1,NK
 		II = WELL_COE(IK)%WELL_CELL_INDEX(JK)
 		WRITE (99,*) NODE_CF(II)%X,NODE_CF(II)%Y,WELL_COE(IK)%CELL_COE(JK)
 	END DO	
 	WRITE (99,*) 'FACE NUM = ',WELL_COE(IK)%FACE_NUM
 	NK = WELL_COE(IK)%FACE_NUM
 	DO JK = 1,NK
 		II = WELL_COE(IK)%WELL_FACE_INDEX(JK)
 		WRITE (99,*) NODE_CF(II)%X,NODE_CF(II)%Y,WELL_COE(IK)%FACE_COE(JK)
 	END DO	
 END DO
 CLOSE (99)
