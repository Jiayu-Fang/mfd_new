...
!-------For the Boundary Conditions
	INTEGER :: WELL_NUM
	REAL (KIND = 8), ALLOCATABLE, DIMENSION (:) :: QWELL,WELL_DEPTH
...
MODULE MATRIX
 ...
 ! For soil properties
 	...
 	REAL (KIND = 8), ALLOCATABLE :: MCF(:,:),MCF1(:,:),PC(:,:),RC(:,:),NC(:,:)
...
!-------For the Input File
	...
	!	WRITE (993,'(100F20.10)') T,(HTH(DRAW_XYZ(N)),N=1,DRAW_NUM)
		...
		ELSE 
			...
			!		OPEN (912,FILE = trim(PATH)//trim(NAME_FILE)//'_velocity_cell_'//CHAR(IK+48)//'.dat',status = 'unknown')
					DO N = 1,DRAW_NUM
						DO JK = 1,CELL_COE(N)%POINT_SIZE
							...
							END IF
						END DO
					!	WRITE (912,'(10F20.10)') CELL_COE(N)%X,CELL_COE(N)%Y,CELL_COE(N)%VX,CELL_COE(N)%VY
					END DO
					CLOSE (995); !CLOSE (912)
				END IF
 			END DO
