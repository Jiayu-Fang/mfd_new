 ...
 DO K = 1,CELL_TOT
 	IF (CELL_COE(K)%POINT_TYPE.NE.(-1)) THEN 
 		...
 		DO IK = 1,FACE_NUM
 			...
 			END DO
 			KK = CELL_COE(K)%POINT_INDEX(IK)
 			VX(IK) = CELL_COE(K)%RC(IK,1)*(FACE_COE(KK)%P)*K_FACE(IK)
 			VY(IK) = CELL_COE(K)%RC(IK,2)*(FACE_COE(KK)%P)*K_FACE(IK)
 		...
 		END DO
 		CELL_COE(K)%VX = CELL_COE(K)%VX/CELL_COE(K)%CELL_V
 		CELL_COE(K)%VY = CELL_COE(K)%VY/CELL_COE(K)%CELL_V
